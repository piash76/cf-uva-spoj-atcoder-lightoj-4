...
int main()
{
    ...
    for(auto x:v1) a1.push_back(x);

    v1.clear();
    gen_factor(b);

    for(auto x:v1) b1.push_back(x);
 ...
 //  for(auto x: c1) cout<<x<<" ";
   ...
   //cout<<endl;
    ...
    while(t--)
    {
        ...
        sc2l(l,r);
        ll ans=-1;
        //ll x=lower_bound(ar,ar+idx,r)-ar;
        //cout<<x<<endl;
        for(i=idx-1;i>=0;i--)
        {
            if(ar[i]>=l and ar[i]<=r) {ans=ar[i]; break;}
        }
