...
int solve(int curindex,int jump)
{
   // cout<<"CUR "<<curindex<<" JUMP "<<jump<<endl;

    if(curindex>30000) return 0;

    if(dp[curindex][jump]!=-1)
    {
        //cy;
        return dp[curindex][jump];
    }

    //cout<<"YES"<<endl;

    int ans=0;
    ans=max(ans,a[curindex]+solve(curindex+jump,jump));
    ans=max(ans,a[curindex]+solve(curindex+jump+1,jump+1));
    if(jump>1) ans=max(ans,a[curindex]+solve(curindex+jump-1,jump-1));

   // cout<<"ANS "<<ans<<endl;

    return dp[curindex][jump]=ans;
...
int main()
{
   ...
   // int ans=a[d];
   // cout<<solve(d,d)<<endl;
    //cout<<solve(d,d+1)<<endl;
   //cout<<solve(d,d-1)<<endl;

   // cout<<max3(solve(d,d),solve(d,d+1),solve(d,d-1))<<endl;
