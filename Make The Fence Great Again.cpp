...
ll dp[maxn][3];
ll n;
ll solve(ll idx,ll prev_add)
{
    //cout<<"IDX "<<idx<<" prev add "<<prev_add<<endl;

    if(idx>=n) {
       // cout<<"IDX "<<idx<<" prev add "<<prev_add<<" n "<<n<<endl;
            return 0; }

    if(dp[idx][prev_add]!=-1)
    {
        //cy;
        return dp[idx][prev_add];
    }
    ...
    for(ll i=0;i<=2;i++)
    {
        if(idx==0 or a[idx-1]+prev_add!=a[idx]+i)
        {
            ans=min(ans,b[idx]*i+solve(idx+1,i));
        }
    }
...
int main()
{
    ...
    while(t--)
    {
       ...
       // mem(dp,-1);
        ...
        for(i=0;i<=n;i++) dp[i][0]=dp[i][1]=dp[i][2]=-1;

        //ll ans=min3(solve(0,0),solve(0,1),solve(0,2));
        ll ans=solve(0,0);
